...
{
	printf("Loading textures with MLX42...\n");
	
	// Load wall texture (PNG)
	...
	printf("Wall texture: %dx%d\n", cub->wall_tex.width, cub->wall_tex.height);
	printf("Space texture: %dx%d\n", cub->space_tex.width, cub->space_tex.height);
	printf("Player texture: %dx%d\n", cub->player_tex.width, cub->player_tex.height);
...
unsigned int	get_texture_pixel(t_texture *tex, int x, int y)
{
	...
	uint32_t	offset = y * tex->tex->width + x;
